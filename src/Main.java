...
//				ch.setType(a[4]);
//				
//				if(!chunks[0].equals("")) for(int i=0;i<chunks.length;i++) ch.addChunk(chunks[i]);
//				if(!tokens[0].equals("")) for(int i=0;i<tokens.length;i++) ch.addToken(tokens[i]);
//				if(!heads[0].equals("")) for(int i=0;i<heads.length;i++) ch.addHead(heads[i]);
//				
//				c.addChunk(ch);
//			}
//			s.close();
...
//			while(s.hasNext()){
//				String[] a = s.nextLine().split(" -- ");
//				String[] chunks = a[2].replace("[", "").replace("]", "").replace(" ", "").split("[,]");
//				Sentence se = new Sentence(a[0]);
//				se.setVoice(a[1].equals("null") ? null : a[1]);
//
//				if(a.length==4 && !a[3].equals("null")) se.setText(a[3]);
//				else se.setText(null);
//				
